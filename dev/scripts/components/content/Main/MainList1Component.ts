  			...
  			<div class="col-md-6" *ngFor="let post of content">
  				<div class="news-post standard-post2">
  					<div class="post-gallery">
  						<a (click)="routing(out(post,'link','value'))"><img src="{{post.value.img[0].value}}" alt="" width="330" height="260"></a>
  					...
  					<div class="post-title">
  						<h2><a (click)="routing(out(post,'link','value'))">{{post.value.tit}}</a></h2>
  						<ul class="post-tags">
                ...
                <li *ngIf="post.value.counter"><i class="fa fa-eye"></i>{{post.value.counter[0].value}}</li>
  					...
  					<div class="post-content">
  						...
  						<a (click)="routing(out(post,'link','value'))" class="read-more-button"><i class="fa fa-arrow-circle-right"></i>Подробнее</a>
  					</div>
  				</div>
  			</div>
  		</div>
